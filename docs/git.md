...
The Git plugin in gitreleaser, by default, does the following:
...
When not in CI mode, gitreleaser will ask for confirmation before each of the commit, tag, and push steps.
...
gitreleaser should be able to do the same.
...
By default, `gitreleaser` uses branch's tracking information, unless there isn't any, in which case it defaults to
...
```bash
gitreleaser minor --git.pushArgs=--follow-tags --git.pushArgs=--force
```
...
This is disabled by default, but gitreleaser can exit the process when the current branch is not as configured:
...
Make sure to commit, stash, or revert the changes before running gitreleaser. In case the currently staged changes
...
branch. So if the current branch is `next` then the full command that gitreleaser will execute becomes
...
example use case and how it can be handled using gitreleaser:
...
- The release of v1.x.x can be done while still in this branch using `gitreleaser --no-git.requireUpstream`.

### No commits

By default, gitreleaser does not check the number of commits upfront to prevent "empty" releases. Configure
`"git.requireCommits": true` to exit the gitreleaser process if there are no commits since the latest tag.
...
[11]: https://github.com/gitreleaser/gitreleaser/issues/350
